...
Data *Clustering::getData() { return samples; }

vector<int> Clustering::getLabels() { return labels; }

vector<double> Clustering::getCostHistory() { return costHistory; }
...
void Clustering::kmean() {
  ...
  while (notConverged) {
    ...
    // Set the position of each cluster to the mean of all data points belonging
    // to that cluster
    fill(totalSamplesInCluster.begin(), totalSamplesInCluster.end(), 0);
    for (int i = 0; i < k; i++) {
      fill(sumDistanceInCluster[i], sumDistanceInCluster[i] + sampleDimension,
           0);
    }
    ...
    if (ddelta < 0.5 || counter++ > maxCounter) {
      ...
      if (counter == maxCounter) {
        cout << "Kmean not converged after " << maxCounter << "  iterations."
             << endl;
      } else {
        cout << "KMean converged after " << counter
             << " iterations, delta= " << ddelta << endl;
      }
    }
...
void Clustering::kmeanInitialize() {
  ...
  for (int i = 0; i < k; i++) {
    centers.push_back(
        vector<double>(samples->getData(rand() % samples->getSize())));
  }
...
void Clustering::kmeanCheckConvergence() {
  ...
  if (ddelta < 0.0001 || counter++ > iterations) {
    ...
    if (counter == iterations) {
      cout << "Kmean not converged after " << iterations << "  iterations."
           << endl;
    } else {
      cout << "KMean converged after " << counter
           << " iterations, delta= " << ddelta << endl;
...
template <class T, class S, class C> S &Container(priority_queue<T, S, C> &q) {
  struct Queue : private priority_queue<T, S, C> {
    static S &Container(priority_queue<T, S, C> &q) { return q.*&Queue::c; }
  };
