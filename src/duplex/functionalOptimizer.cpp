...
void FunctionalOptimizer::initialize() {
  ...
  for (int i = 1; i < pathSegments - 1; i++) {
    ...
    // p[0] = i;    //time, ignore this, the states are no longer time-annotated in
    // dido case
...
double FunctionalOptimizer::score(State *state, double *maxBound,
                                  double *minBound) {
  double distance = 0;
  ...
  // 1. boundary: equivalent to the boundary conditions in BVPs. The closer we
  // get to the boundary value, the better  2. boundary-strict: similar to
  // boundary, but if we are crossing the bounary value the sameples is not
  // usefull anymore.
  //   for example, for the length of the curve we use bounadry-hard objectives,
  //   so the length is strictly less than boundary
  // 3. maximize
  // 4. minimize
  for (int i = 0; i < objectiveSize; i++) {
    auto potentialdistance =
        state->distance(2, state->getParameter(), boundary);
    auto potentialsum =
        (state->getParameter()[1] * (b - state->getParameter()[0])) / 2.0;
    double normalizedDistance = (objectives[i] - goals[i]) / (max[i] - min[i]);
    normalizedDistance *= normalizedDistance;
    if ((objectiveType[i] == "boundary-strict") &&
        (objectives[i] > opt[i]) // if the length of the curve is bigger than
                                 // the strict boundary, discard this trace
        && (objectives[i] + potentialdistance >
            opt[i]) // evaluating the potential of this sample: if we even take
                    // the direct route to the boundary
        // condition and still the length is more than the bondary, discard this
        // sample
        ) {
