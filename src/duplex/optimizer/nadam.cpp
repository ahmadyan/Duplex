...
bool Nadam::hasGradientInformation(){
    return true;
}
State* Nadam::update(State* u){
    ...
    auto momentum_cache_t = exponential_decay_rates_beta1 * (1.0 - 0.5 * pow(0.96, iterations * schedule_decay));
    auto momentum_cache_t_1 = exponential_decay_rates_beta1 * (1. - 0.5 * (pow(0.96, (iterations + 1) * schedule_decay)));
    ...
    for(int i=0;i<parameterDimension;i++){
        ...
        m_t[i] = exponential_decay_rates_beta1 * moment[i] + (1. - exponential_decay_rates_beta1) * dx[i];
        m_t_prime[i] = m_t[i] / (1. - m_schedule_next);
        v_t[i] = exponential_decay_rates_beta2 * velocity[i] + (1.0 - exponential_decay_rates_beta2) * dx[i]*dx[i];
        v_t_prime[i] = v_t[i] / (1. - pow(exponential_decay_rates_beta2, iterations));
        m_t_bar[i] = (1. - momentum_cache_t) * g_prime[i] + momentum_cache_t_1 * m_t_prime[i];
    }
    
    for(int i=0;i<parameterDimension;i++){
        moment[i]=m_t[i];
        velocity[i]=v_t[i];
        input[i] = prev[i] - learning_rate * m_t_bar[i] / (sqrt(v_t_prime[i]) + fudgeFactor);
    }
