...
Adam::Adam(Settings *s) : Optimizer(s) {
  ...
  cout << "Initializing Adam optimizer complete"
       ...
       << ", bias correction= " << bias_correction << ", adamax= " << adamax
       << endl;
}

Adam::~Adam() {}

bool Adam::hasGradientInformation() { return true; }

void Adam::update_adam(double *input) {
  // lr_t = self.lr * K.sqrt(1. - K.pow(self.beta_2, t)) / (1. -
  // K.pow(self.beta_1, t))
  auto learningRate =
      learning_rate_base * sqrt(1 - decay_beta2) / (1 - decay_beta1);

  // Update biased first and second moment estimate
  // m = beta1*m + (1-beta1)*dx
  // v = beta2*v + (1-beta2)*(dx**2)
  for (int i = 0; i < parameterDimension; i++) {
    moment[i] = exponential_decay_rates_beta1 * moment[i] +
                (1 - exponential_decay_rates_beta1) * dx[i];
    velocity[i] = exponential_decay_rates_beta2 * velocity[i] +
                  (1 - exponential_decay_rates_beta2) * dx[i] * dx[i];
  }
...
void Adam::update_adamax(double *input) {
  ...
  for (int i = 0; i < parameterDimension; i++) {
    moment[i] = exponential_decay_rates_beta1 * moment[i] +
                (1 - exponential_decay_rates_beta1) * dx[i];
    // velocity is the variable u in the paper, Update the exponentially
