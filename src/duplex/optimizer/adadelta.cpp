...
Adadelta::Adadelta(Settings* s):Optimizer(s){
    ...
    accelerated_adaDelta = settings->check("optimization.adadelta_acceleration", "enable");
    ...
    cout << "Initializing Adadelta optimizer complete"
        ...
        << ", accelerated=" << accelerated_adaDelta
        << endl;
...
bool Adadelta::hasGradientInformation(){
    return true;
}
...
State* Adadelta::update(State* u){
    ...
    for(int i=0;i<parameterDimension;i++){
        ...
        accumulator[i] = autocorrelation*accumulator[i] + (1-autocorrelation)*dx[i]*dx[i];
        // use the new accumulator and the *old* delta_accumulator
        step[i] = dx[i]*sqrt(delta[i] + fudge_factor) / sqrt(accumulator[i] + fudge_factor);
        // update delta accumulator
        if(accelerated_adaDelta){
            //using the gradient information to make bigger jumps
            delta[i] = autocorrelation*delta[i] + (1-autocorrelation)*step[i]*dx[i];
        }else{
            // original adadelta equation:
            delta[i] = autocorrelation*delta[i] + (1-autocorrelation)*step[i]*step[i];
        }
        // apply update
