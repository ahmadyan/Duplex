...
GradientDescent::GradientDescent(Settings* s):Optimizer(s){
    ...
    cout << "Initializing Gradient Descent optimizer complete"
         ...
         << " , Momentum=" << momentum
         << " , Decay rate=" << decay
         << " , learning rate=" << learning_rate_base << endl;
}
...
State* GradientDescent::update(State* u){
    ...
    // we store the previous value of velocity to be used in Nestrov's momentum calculation
    ...
    for(int i=0;i<u->getParameterSize();i++){
        ...
        if(nestrov){
            ...
            input[i] = prev[i] - momentum*previous_velocity + (1+momentum)*velocity;
        }else{
