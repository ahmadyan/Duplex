...
State *Search::getOptimum() { return pq.top(); }

unsigned long Search::getSize() { return db.size(); }
...
State *Search::getState(int i) { return db[i]; }

// returns the last state that was inserted to data-base
State *Search::getState() { return db[db.size() - 1]; }

void Search::setState(State *s, int i) { db[i] = s; }
...
void Search::range(State *s) {
  ...
  // double range);  presults = kd_nearest_range(kd, q_sample->get(), 0.01);
