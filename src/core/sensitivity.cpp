...
void Sensitivity::setInputOutputMinMax(double *iMin, double *iMax, double *oMin,
                                       double *oMax) {
  inputMin = iMin;
  inputMax = iMax;
  outputMin = oMin;
...
void Sensitivity::commit() {
  ...
  for (int i = 0; i < inputStack.size(); i++) {
    for (int j = 0; j < outputStack.size(); j++) {
      sensitivityMatrix[inputStack[i]][outputStack[j]] +=
          abs(outputDeltaStack[j]);
    }
  }
  inputStack.clear();
...
void Sensitivity::generateSensitivityMatrix() {
  ...
  for (int i = 0; i < inputSize; i++)
    max[i] =
        *max_element(sensitivityMatrix[i], sensitivityMatrix[i] + outputSize);
