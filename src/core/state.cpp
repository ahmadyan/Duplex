...
void State::setObjective(double v, int i) { objectiveVector[i] = v; }
...
double *State::getParameter() { return parameterVector; }

double *State::getObjective() { return objectiveVector; }

StateType State::getType() { return type; }

void State::setType(StateType t) { type = t; }
...
double State::unifRand() { return rand() / double(RAND_MAX); }

double State::unifRand(double a, double b) { return (b - a) * unifRand() + a; }
...
int State::getParentID() { return parentID; }

void State::setParentID(int pid) { parentID = pid; }
...
State *State::getParent() { return parent; }

int State::getID() { return id; }

void State::setID(int i) { id = i; }

int State::getParameterSize() { return parameterDimension; }

int State::getObjectiveSize() { return objectiveDimension; }
...
double *State::getRewardVector() { return reward; }

double State::getRewardCDF() { return rewardCDF; }
...
double State::getScore() { return score; }

void State::setScore(double s) { score = s; }

void State::save(boost::property_tree::ptree *pt) {
  stringstream param, obj, rew;
  for (int i = 0; i < parameterDimension; i++)
    param << parameterVector[i] << " ";
  for (int i = 0; i < objectiveDimension; i++)
    obj << objectiveVector[i] << " ";
  for (int i = 0; i < parameterDimension; i++)
    rew << reward[i] << " ";
...
double State::getDerivative(int i, int j) { return jacobian[i][j]; }

vector<double> State::getDerivativeVector(int i) { return jacobian[i]; }

vector<vector<double>> State::getJacobian() { return jacobian; }

void State::setJacobian(vector<vector<double>> j) { jacobian = j; }

// convert the double* grad into vector<double> and pushes it into the jacobian
// matrix assumes only one objective deletes the grad array afterward
