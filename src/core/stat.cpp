...
Stat::Stat(Settings *s, double *_max, double *_min, double *_opt) {
  ...
  sensitivity =
      new Sensitivity(parameterDimension, objectiveDimension,
                      (double)settings->lookupFloat(
                          "sensitivity-analysis.relativeSensitivityThreshold"));
}
...
void Stat::updateBias(State *q) { bias.push_back(q); }

void Stat::update(int i, State *goal, State *qsample, State *qnear, State *qnew,
                  double score) {
...
void Stat::updateReward(State *goal, State *qnear, State *qnew) {
  ...
  if (reward[nextCandidateParameter] < minAward)
    reward[nextCandidateParameter] = minAward;
  if (reward[nextCandidateParameter] > maxAward)
    reward[nextCandidateParameter] = maxAward;
...
double Stat::getError(int i) { return error[i]; }

double Stat::getDistance(int i) { return currentDistance[i]; }

void Stat::error_push_back(double v) { error.push_back(v); }

void Stat::distance_push_back(double v) { currentDistance.push_back(v); }

vector<double> Stat::getError() { return error; }

vector<double> Stat::getCurrentDistance() { return currentDistance; }
