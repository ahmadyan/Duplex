...
System::System(Settings *s) {
  ...
  // We support two types of numerical simulator. 1) Synopsys HSPICE and 2) our
  // internal simulator
  if (config->check("simulation.engine", "hspice")) {
    ...
    // for using the internal simulator, the function has to be explicitly
    ...
    if (config->check("objective.gradient", "true")) {
      ...
      // functions are listed as uid- objects, so they might have numbering
      // embedded in them, like uid-00001-objective The functionVariable is the
      // "true" name of the function in the config file
      auto functionVariablesNames =
          config->listVariables("objective", "uid-objective");
      for (auto fvariableName : functionVariablesNames) {
        auto derivativeVariablesNames = config->listVariables(
            config->concatScope("objective", fvariableName).c_str(),
            "uid-grad.derivative");
        vector<string> derivatives;
        for (auto dVariableName : derivativeVariablesNames) {
          string derivate = config->lookupString(
              config->concatScope("objective", fvariableName, dVariableName)
                  .c_str());
...
void System::eval(State *s) { return eval(s, 0); }

double evaluteExpression(string expression, vector<string> variables,
                         double *values) {
...
void System::eval(State *s, double t) {
  if (type == HSPICE) {
    ...
    engine->generateNetlist(parameterName, parameterUnit, s->getParameter(),
                            setting);
    engine->runSimulation(simulationLogFilename);
    double *result =
        engine->parseSimulationLog(simulationLogFilename, objectives);
  ...
  if (type == INTERNAL) {
    ...
    for (int i = 0; i < objectiveSize; i++) {
      if (functions[i].at(0) == '#') {
        // todo: needs refactoring, come up with a better way to evaluate
        // integralss idea: put integrals as an operator inside the config file
        if (s->getParentID() == -1) {
          ...
          if (functions[i] == "#L") {
            ...
            double dL = s->distance(s->getParameterSize(), parameters,
                                    s->getParent()->getParameter());
            objectives[i] = fprev + dL;
          } else if (functions[i] == "#A") {
            //#A is hardcoded for the area of the curve
            double fprev = s->getParent()->getObjective()[i];
            double dx =
                abs(s->getParent()->getParameter()[0] - s->getParameter()[0]);
        ...
        if (jacobian.size() > 0) {
          ...
          for (int i = 0; i < jacobian.size(); i++) {
            ...
            for (int j = 0; j < jacobian[i].size(); j++) {
              auto dfidj =
                  evaluteExpression(jacobian[i][j], variables, parameters);
...
string System::getFunction(int i) { return functions[i]; }
