...
void Hspice::generateNetlist(vector<string> parameterName,
  ...
  for (int i = 0; i < parameterName.size(); i++) {
    sed << "-e s/" << parameterName[i] << "/" << parameters[i]
        << parameterUnit[i] << "/ ";
  }
  /*if (dcSimulation){
          sed << "-e s/$tran/" << "\".tran " << dt / 10 << " " << dt << "\"/ ";
          sed << "-e s/$save/" << "\".save type=ic file=" << setting[0] << "
  level=all time=" << dt << "\"/ "; }else{ sed << "-e s/$tran/" << "\".tran " <<
  dt / 10 << " " << dt << " uic" << "\"/ "; sed << "-e s/$save/" << "\".save
  type=ic file=" << setting[0] << " level=all time=" << dt << "\"/ "; sed << "-e
  s/$load/" << "\".load file=" << setting[1] << "\"/";
  }*/
...
void Hspice::runSimulation(string simulationLogFilename) {
  string hspiceCommand =
      "hspice " + netlistFile + " > " + simulationLogFilename;
...
double *Hspice::parseSimulationLog(string filename, vector<string> objectives) {
  ...
  for (int i = 0; i < objectives.size(); i++) {
    const string floating_point_regex =
        "([-+]?[0-9]*\\.?[0-9]+)[eE]([-+]?[0-9]+)?";
    const string whitespace_regex = "^[ \t]*";
    const string extraspace_regex = "[ ]*";
    string regexString = whitespace_regex + objectives[i] + "=" +
                         extraspace_regex + floating_point_regex;
    regex reg(regexString);
    regs.push_back(reg);
  }

  // parse the logfile, for each line, check it against all the
  // regular-expressions
