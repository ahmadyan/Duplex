...
// temporary code, visual studio 2015 refactored CRT libraries. Now, some codes
// (specifically config4cpp) will produce link libraries because they cannot
// find _iob_func (used in _sprintf). one solution is to recompile them using
// visual studio 2015 with legacy_stdio_definitions.lib additional dependency.
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    desc.add_options()("help", "Print help messages")(
        ...
        "Specifies the configuration file")("like", "this");
    po::variables_map vm;
    try {
      ...
      if (vm.count("help")) {
        std::cout << "Duplex optimization engine. " << std::endl
                  << "Version 0.1    (C) 2015 Seyed Nematollah (Adel) "
                     "Ahmadyan, All right reserved."
                  << std::endl
                  << desc << std::endl;
        return SUCCESS;
      }
    ...
    cout << "Parsing arguments complete. Proceed to parsing config file ..."
         << endl;
    ...
    log << "Parsing config file " << vm["config"].as<std::string>().c_str()
        << " complete." << endl;
    ...
    if (settings->check("plot.enable", "true")) {
      ...
      for (int i = 0; i < plots.size(); i++) {
        Graphics *graphic =
            new Graphics(settings->lookupString("plot.gnuplot"));
