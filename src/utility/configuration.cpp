...
//  Complete overhaul on 10/21/15. Replaced my own configuration class with
//  config4cpp library.
...
int strcpy_s(char *strDestination, size_t numberOfElements,
...
SettingsException::~SettingsException() { delete[] m_str; }

const char *SettingsException::what() const { return m_str; }
...
void Settings::parse(const char *cfgSource,
                     const char *scope) throw(SettingsException) {
...
const char *Settings::lookupString(const char *name) const
...
void Settings::lookupList(const char *name, const char **&array,
...
int Settings::lookupDurationMilliseconds(const char *name) const
    throw(SettingsException) {
  Configuration *cfg = (Configuration *)m_cfg;
  try {
...
int Settings::lookupDurationSeconds(const char *name) const
    throw(SettingsException) {
  Configuration *cfg = (Configuration *)m_cfg;
  try {
...
bool Settings::check(const char *name, const char *value) const
    throw(SettingsException) {
  Configuration *cfg = (Configuration *)m_cfg;
  try {
...
// list the values for all the uid-elements matching the uid name. the scope is
// defined as m_scope.name for example, parameter.uid-parameter.name --> returns
// the names for all parameter.uid-parameter
vector<string> Settings::listValues(const char *name, const char *uid) const
    throw(SettingsException) {
  Configuration *cfg = (Configuration *)m_cfg;
  vector<string> list;
  char scope[100];
  ...
  try {
    cfg->listLocallyScopedNames(
        m_scope, name, Configuration::CFG_SCOPE_AND_VARS, true, result);
...
// for example, parameter.uid-parameter.name --> returns
// parameter.uid-000000000-parameter.name,
// parameter.uid-000000001-parameter.name, etc. note this doesn't return the
// actual name, instead it just says what the uid variable is listed as, should
// use lookup afterward.
vector<string> Settings::listVariables(const char *name, const char *sub) const
    throw(SettingsException) {
  Configuration *cfg = (Configuration *)m_cfg;
  vector<string> list;
  config4cpp::StringVector result;
...
string Settings::concatScope(string s0, string s1) { return s0 + "." + s1; }
...
void Settings::test() const throw(SettingsException) {
  ...
  config4cpp::StringVector result;
  try {
    // cfg->listLocallyScopedNames(m_scope, "",
    // Configuration::CFG_SCOPE_AND_VARS, true, result);
    cfg->listFullyScopedNames(m_scope, "", Configuration::CFG_SCOPE_AND_VARS,
                              true, result);
    for (int i = 0; i < result.length(); i++) {
