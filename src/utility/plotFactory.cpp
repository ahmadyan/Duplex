...
PlotFactory::PlotFactory(Settings *s, Duplex *_duplex,
                         Clustering *_clustering) {
  settings = s;
  if (duplex)
    stat = _duplex->getStat();
  duplex = _duplex;
  clustering = _clustering;
}
...
string PlotFactory::drawCanvas(string function, double xmin, double xmax,
                               double ymin, double ymax, string plotType,
                               string title) {
  stringstream canvas;
  if (plotType == "default") {
    canvas << "plot [" << xmin << ":" << xmax << "][" << ymin << ":" << ymax
           << "] 0 title '" << title
           << "' with linespoints lt \"white\" pt 0.01\n";
  } else {
    canvas << "reset\n"
    ...
    if (plotType == "contour") {
      canvas << "p 'test.dat' with image, 'cont.dat' with linespoints lt "
                "\"white\" pt 0.01\n";
...
string PlotFactory::drawParameterTree(int x, int y, int z,
                                      string sizingPreference, string plotType,
                                      string title) {
  auto db = duplex->getDB();
  string color = "blue";
  stringstream cmdstr;
  double xmin = 99, xmax = -99, ymin = 99, ymax = -99;
  for (int i = 1; i < db->getSize();
       i++) { // i starts at 1 because we ignore the root
    ...
    cmdstr << " set arrow from " << iFromX << "," << iFromY << "," << iFromZ
           << " to " << iToX << "," << iToY << "," << iToZ
           << " nohead  lc rgb \"" << color << "\" lw 2 front \n";
  }
  ...
  stringstream board;
  board << drawCanvas(duplex->getSystem()->getFunction(0), xmin, xmax, ymin,
                      ymax, plotType, title);
  board << cmdstr.str();
  return board.str();
}
...
string PlotFactory::drawObjectiveTree(int x, int y, string title) {
  ...
  for (int i = 1; i < db->getSize();
    ...
    xmax = maximum(iFromX, iToX, xmax);
    ymax = maximum(iFromY, iToY, ymax);
    cmdstr << " set arrow from " << iFromX << "," << iFromY << " to " << iToX
           << "," << iToY << " nohead  lc rgb \"" << color << "\" lw 2 \n";
  }

  stringstream board;
  board << "plot [" << xmin << ":" << xmax << "][" << ymin << ":" << ymax
        << "] 0  title '" << title << "' with linespoints lt \"white\" pt 0.01";
  cmdstr << board.str() << "\n " << cmdstr.str();
  return cmdstr.str();
}
...
string PlotFactory::drawTrace(int x, int y, string title) {
  ...
  cout << "optimum state = " << s->getID() << ", " << s->getObjective()[0]
       << " , " << s->getObjective()[1] << ", " << s->getObjective()[2] << endl;
  ...
  s = db->getState(index);
  cmdstr << " set arrow from " << 50 << "," << 0 << " to "
         << s->getParameter()[x] << "," << s->getParameter()[y]
...
string PlotFactory::plotError() {
  ...
  stringstream cmdstr;
  cmdstr << "plot [" << 0 << ":" << error.size() << "][" << 0 << ":"
         << 1.1 * error[0] << "] 0 with linespoints lt \"white\" pt 0.01"
         << endl;
  for (int i = 1; i < error.size(); i++) {
    cmdstr << " set arrow from " << i - 1 << "," << error[i - 1] << " to " << i
           << "," << error[i] << " nohead  lc rgb \"red\" lw 2 \n";
  }
  return cmdstr.str();
}
...
string PlotFactory::plotDistance() {
  ...
  stringstream cmdstr;
  cmdstr << "plot [" << 0 << ":" << currentDistance.size() << "][" << 0 << ":"
         << 2.0 * currentDistance[0]
         << "] 0 with linespoints lt \"white\" pt 0.01" << endl;
  for (int i = 1; i < currentDistance.size(); i++) {
    cmdstr << " set arrow from " << i - 1 << "," << currentDistance[i - 1]
           << " to " << i << "," << currentDistance[i]
...
string PlotFactory::getPlot(int plotID) {
  ...
  string type =
      settings->lookupString(("plot." + plots[plotID] + ".type").c_str());
  if (type == "error") {
    return plotError();
    ...
    auto plotPreference =
        settings->lookupString(("plot." + plots[plotID] + ".plot").c_str());
    auto sizingPreference =
        settings->lookupString(("plot." + plots[plotID] + ".size").c_str());
    return drawParameterTree(x, y, z, sizingPreference, plotPreference,
                             "param");
    /*
    if((x>parameterDimension-1) || (y>parameterDimension-1) ||
    (z>objectiveDimension-1)){ cout << "error: tree.parameter plotting error,
    configuration file contains an invalid parameter x,y,z " << endl ; return
    ""; }else{
