...
class Log {
  ...
  using endl_type =
      ostream &(ostream &); // prototype for std::endl template function

public:
  static const std::string default_log_header;

  Log(const std::string &log_header = default_log_header,
  ...
  template <typename T> Log &operator<<(const T &data) {
    ...
    struct tm now_tm;
#ifdef _WIN32
    localtime_s(&now_tm, &now_time_t); // localtime_s is the c++11, older
                                       // compilers should be OK with localtime_r
#else
    localtime_r(&now_time_t, &now_tm);
#endif
    if (newline)
      out << _log_header << "(" << now_tm.tm_hour << ":" << now_tm.tm_min << ":"
          << now_tm.tm_sec << "): " << data;
    else
      out << data;
    newline = false;
