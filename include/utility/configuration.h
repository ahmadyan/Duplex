...
public:
  Settings();
  ~Settings();
  void parse(const char *cfgSource,
             const char *scope = "") throw(SettingsException);
  bool check(const char *name, const char *value) const
  ...
  virtual int lookupDurationSeconds(const char *name) const
      throw(SettingsException);
  vector<string> listVariables(const char *name, const char *) const
      throw(SettingsException);
  vector<string> listValues(const char *name, const char *) const
      throw(SettingsException);
